...
# from ppimm.models.vision_transformer import Mlp, PatchEmbed,  _cfg
from cait.patch_embed import PatchEmbed
from cait.mlp import Mlp
from cait.drop import DropPath
...
class Class_Attention(nn.Layer):
    ...
    def forward(self, x):
        ...
        # attn.shape: [B, self.num_heads, 1, N]
...
class LayerScale_Block_CA(nn.Layer):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        gamma_1 = self.create_parameter(shape=[dim], default_initializer=paddle.nn.initializer.Constant(init_values))
        self.add_parameter("gamma_1", gamma_1)
        gamma_2 = self.create_parameter(shape=[dim], default_initializer=paddle.nn.initializer.Constant(init_values))
...
class Attention_talking_head(nn.Layer):
    ...
    def forward(self, x):
        ...
        # the shape of q, k, v:  [B, self.num_heads, N, C // self.num_heads]
...
class cait_models(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.add_parameter("pos_embed", pos_embed)
        cls_token = self.create_parameter(
            shape=[1, 1, embed_dim], default_initializer=zeros_)
        self.add_parameter("cls_token", cls_token)
